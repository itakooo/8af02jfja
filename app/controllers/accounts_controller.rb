...
class AccountsController < ApplicationController
  ...
  def mypage
    ...
    @emps = Emp.where("account_id == ?", session[:usr]).page.order("created_at DESC").page(params[:page]).per(20)
    @unis = Uni.where("account_id == ?", session[:usr]).page.order("created_at DESC").page(params[:page]).per(20)
  end
